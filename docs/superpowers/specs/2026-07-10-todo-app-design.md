...
**사용자가 친 글자를 FTS5 질의 구문으로 해석하지 않습니다.** 검색창에 `"배포"`라고 따옴표를 붙이거나 `(`를 치는 건 사람이 늘 하는 일인데, 그대로 넘기면 `unterminated string` 같은 오류가 납니다. `AND`, `OR`, `NOT`, `NEAR`, `*`, `^`, `:` 도 전부 구문으로 먹힙니다. 커맨드 팔레트는 키를 누를 때마다 검색하므로, 입력 중간 상태에서 계속 터집니다.

그래서 질의문은 코어가 직접 조립합니다. 입력을 공백으로 쪼개고, 각 토큰을 큰따옴표로 감싸되 안에 있는 큰따옴표는 두 개로 이스케이프합니다. 그리고 마지막 토큰에만 `*`를 붙여 접두 검색으로 만듭니다. 타이핑하는 도중에도 결과가 따라옵니다. 쓸 만한 토큰이 하나도 안 남으면 질의하지 않고 빈 목록을 돌려줍니다.

어떤 입력이 들어와도 검색은 오류를 내지 않습니다.
...
`todo`, `in_progress`, `done`, `deferred` 넷입니다. 전이에 제약이 없습니다. 어디서 어디로든 갑니다.

`done`으로 갈 때 `completed_at`을 찍고, `done`에서 나올 때 지웁니다. `done`으로 갔고 그 todo에 Linear 링크가 있으면 같은 트랜잭션에서 아웃박스에 `linear_complete`를 넣습니다.

할 일을 만들 때 초기 상태를 지정할 수 있습니다. 기본은 `todo`입니다. Linear에서 가져온 이슈는 `in_progress`로 만들어져야 하므로(7절), 만들고 나서 상태를 바꾸는 두 번의 쓰기가 아니라 한 번에 끝냅니다. `done`으로 바로 만들면 `completed_at`도 같이 찍습니다.

### 보류(deferred)

잠깐 신경 끄고 싶은 할 일을 모으는 상태입니다. `deferred_until`(날짜) 한 칸이 붙습니다. 날짜가 있으면 그날 다시 돌아오고, 비어 있으면(NULL) 계속 보류입니다. 자연어로 입력받아 마감일과 같은 파서를 씁니다.

**깨우기는 읽기가 한다.** `list_todos`는 읽기 전에 `deferred_until`이 오늘이거나 지난 보류 항목을 `todo`로 되돌립니다(`deferred_until`은 NULL로). 한 번의 `UPDATE`로 끝냅니다. 그래서 REST·MCP·창 어디로 읽어도 똑같이 깨어나고, 클라이언트가 따로 할 일이 없습니다. 무기한(NULL) 보류는 절대 깨어나지 않습니다.

**`deferred`를 벗어나면 `deferred_until`은 항상 NULL이 된다.** 보류 항목을 `done`으로 걸면 done이 되고 복귀일은 지워집니다. 링크돼 있으면 여느 done처럼 Linear로 밀립니다. 보류 자체는 Linear로 아무것도 밀지 않습니다. done이 아니기 때문입니다.

깨어난 항목은 `todo`로 돌아옵니다. 보류에 들어가기 전 상태는 기억하지 않습니다.

일반 목록과 필터(전체·todo·진행·완료)는 보류를 뺍니다. 보류는 별도 레인으로만 봅니다(6절). Linear에서 가져온 이슈는 보류로 만들지 않습니다.
...
**소프트 삭제된 할 일은 복구 외의 어떤 변경도 받지 않습니다.** 조회도, 수정도, 상태 변경도, Linear 링크도 전부 `NotFound`입니다. 목록과 검색에서도 빠집니다. 영구 삭제는 만들지 않습니다.

이 규칙이 빠지면 실제 피해가 납니다. 지운 할 일에 done을 걸 수 있으면 그게 아웃박스에 쌓이고, 워커가 Linear로 밀어버립니다. 내가 지운 일이 남의 화면에서 완료됩니다. 그래서 이 조건은 별도 조회가 아니라 `UPDATE`문의 `WHERE`절에 들어가야 합니다. 읽고 나서 쓰면 그 사이에 경합이 생깁니다.

`restore_todo`만이 삭제된 행에 동작합니다.

### 제목

제목은 앞뒤 공백을 잘라내고 저장합니다. 잘라낸 뒤 빈 문자열이면 `InvalidInput`입니다. 생성과 수정 모두 같습니다.
...
상단에 탭 바가 있습니다. 지금은 Todo 탭 하나뿐입니다. 그 아래 필터 칩(todo / in progress / done / 전체), 그 아래 목록, 목록 맨 아래에 보류 레인입니다. 상세는 목록 오른쪽에 붙는 패널로 엽니다.

화면 문구는 한국어입니다. 상태 표시도 `할 일 / 진행 중 / 완료 / 보류`로 보여줍니다. 다만 DB·REST·MCP에 오가는 값은 `todo / in_progress / done / deferred` 그대로입니다. 표시와 저장을 나눕니다.

목록은 TanStack Virtual 계열의 가상화 없이 시작합니다. 개인용이라 수천 건이 되기 전에는 필요 없습니다. 프로파일링으로 느려지는 게 확인되면 그때 넣습니다.

### 보류 레인

목록 맨 아래에 `▸ 보류 N` 헤더가 있습니다. 기본은 접혀 있습니다. 헤더를 클릭하거나 `g`를 누르면 펼쳐집니다. 접힘 상태는 localStorage에 기억합니다. 레인 안은 복귀일 빠른 순으로 늘어놓고, 무기한(복귀일 없음)은 맨 뒤입니다. 보류가 없으면 헤더도 안 보입니다.
...
| `j` `k` | 아래·위 이동 |
| `Enter` | 상세 열기 |
| `Esc` | 상세 닫기 / 편집 취소 / 선택 해제 / 조합 취소 |
| `c` | 새 todo. 인라인 입력. Enter로 저장, Shift+Enter로 저장 후 연속 입력 |
| `e` | 제목 인라인 편집 |
| `d` | done 토글 |
| `i` | in progress 토글 |
| `s` | 보류 토글. 보낼 땐 복귀 날짜 입력 (자연어, 비우면 계속 보류). 보류 항목에서 누르면 바로 풀림 |
...
| `1` `2` `3` `4` | 필터: todo / in progress / done / 전체 |
| `g` | 보류 레인 펼치기·접기 |
| `/` | 검색 |
| `u` | 되돌리기 |
...
`p` 다음 `u` 같은 두 타 조합이 있으니 입력기는 대기 상태를 갖습니다. `p`를 누르면 다음 키 하나를 기다립니다. 만료 시간은 없습니다. 다음 키가 우선순위면 적용하고, 아니면(`Esc` 포함) 조합만 취소하고 아무 일도 하지 않습니다. vim이 `d`를 누른 뒤 하는 것과 같습니다.

만료를 두지 않는 이유가 있습니다. `u`는 조합 밖에서 되돌리기입니다. 조합에 만료를 두면, `p`를 누르고 조금 늦게 `u`를 눌렀을 때 우선순위 대신 직전 작업이 조용히 되돌아갑니다. 파괴적인 오작동이라 만료를 없앱니다.
...
키를 담은 요청 구조체에 `Debug`를 파생하지 않습니다. 어딘가에서 `{:?}` 한 번이면 키가 로그로 샙니다. 손으로 가린 `Debug`를 답니다.

키체인을 열지 못하는 기기가 있습니다. GUI 없는 리눅스 개발 서버가 그렇고, 잠긴 키체인도 그렇습니다. **키를 읽지 못하는 것은 키가 없는 것과 같이 다룹니다.** 읽기 실패를 500으로 올리면 "Linear를 쓰고 있느냐"는 질문에조차 답할 수 없습니다.

그래서 `GET /api/v1/linear/status`는 키가 없어도, 키체인을 못 열어도 항상 200입니다. `configured`와 `key_store_available`을 함께 돌려주어 UI가 둘을 구분합니다. 키가 필요한 동작(링크, 가져오기, 재시도)은 409 `linear_not_configured`입니다.

키를 저장하는 경로는 다릅니다. 사용자가 키를 넣으려는데 못 쓰면 그건 진짜 실패입니다. 조용히 삼키지 않고 알려줍니다.
...
`todo-server`는 라이브러리인 동시에 바이너리입니다. `cargo run -p todo-server`로 Tauri 없이 띄울 수 있습니다.

이게 없으면 M3에서 UI를 개발할 수 없습니다. 개발 기기에 GUI가 없으므로 브라우저에서 Vite dev server를 보고, 데이터는 이 서버에서 받아야 하기 때문입니다.

개발 모드에서는 두 가지가 달라집니다.

**CORS.** Vite dev server는 다른 포트에서 돕니다. 그래서 `--dev-origin http://localhost:2471` 같은 플래그로 그 출처 하나만 허용 목록에 넣습니다. 플래그를 안 주면 CORS는 완전히 닫힙니다. 배포된 Tauri 앱의 웹뷰는 REST를 부르지 않고 IPC를 쓰므로 CORS가 필요 없습니다.

**토큰.** 개발 중에는 브라우저 JS가 Bearer 토큰을 알아야 합니다. Vite 환경변수(`VITE_TODO_TOKEN`)로 주입합니다. 이건 개발 전용 경로입니다. 배포된 앱은 IPC를 쓰므로 웹뷰에 토큰이 없습니다.

토큰이 브라우저에 노출되는 것이 개발 모드에서만 일어난다는 점이 중요합니다. 프로덕션 웹뷰에 토큰을 심으면 안 됩니다.
...
- `p` 다음 `u`가 Urgent를 만드는지 (시간이 얼마가 지나든)
- 조합이 없을 때 `u`는 되돌리기이지 Urgent가 아닌지
- `p` 다음 엉뚱한 키가 조합만 취소하는지
